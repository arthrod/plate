...
describe('POST /api/ai/copilot (template)', () => {
  ...
  describe('Happy Path', () => {
    it('should successfully generate completion with valid request', async () => {
      ...
      expect(response.status).toBe(200);
      expect(mockGenerateObject).toHaveBeenCalledTimes(1);
      expect(data).toEqual({ text: 'suggested completion text' });
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle completion with only text field', async () => {
      mockGenerateObject.mockImplementationOnce(async () => ({
        object: { text: 'simple text' },
      }));
      ...
      expect(data).toEqual({ text: 'simple text' });
    });

    it('should handle completion with empty text', async () => {
      mockGenerateObject.mockImplementationOnce(async () => ({
        object: { text: '' },
      }));
  ...
  describe('Response Format Differences', () => {
    it('should return only object property, not full result', async () => {
      const mockResult = {
        object: { text: 'test completion' },
      ...
      expect(data).not.toHaveProperty('usage');
      expect(data).not.toHaveProperty('finishReason');
      expect(data).not.toHaveProperty('rawResponse');
    });

    it('should not preserve metadata from generateObject', async () => {
      const mockResult = {
      ...
      expect(data).not.toHaveProperty('finishReason');
      expect(data).toEqual({ text: 'completion' });
    });

    it('should handle result.object being undefined gracefully', async () => {
      mockGenerateObject.mockImplementationOnce(async () => ({
        object: undefined,
      }));
  ...
  describe('Backward Compatibility', () => {
    it('should maintain the same API contract for response', async () => {
      mockGenerateObject.mockImplementationOnce(async () => ({
        object: { text: 'completion text' },
        usage: { promptTokens: 10, completionTokens: 5 },
      }));
      ...
      // Should return simplified object with just text property
      expect(data).toHaveProperty('text');
