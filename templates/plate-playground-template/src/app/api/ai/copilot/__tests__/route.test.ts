...
describe('POST /api/ai/copilot (Template)', () => {
  ...
  describe('Happy Path', () => {
    ...
    it('should return only the object property from generateObject result', async () => {
      const mockResult = {
        object: { text: 'extracted text' },
      ...
      // Should only return the object property, not the full result
      expect(data).toEqual({ text: 'extracted text' });
    ...
    it('should handle gpt-3.5-turbo model correctly', async () => {
      ...
      expect(data).toEqual({ text: 'turbo completion' });
      expect(mockedGenerateObject).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('should use shorthand prompt assignment (not prompt: prompt)', async () => {
      const mockResult = { object: { text: 'test' } };
      mockedGenerateObject.mockResolvedValue(mockResult);

      const testPrompt = 'shorthand test prompt';
      const req = createMockRequest({
        model: 'gpt-4',
        prompt: testPrompt,
        system: 'test',
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle empty text in completion object', async () => {
    ...
    it('should handle whitespace-only text in completion', async () => {
      const mockResult = { object: { text: '   ' } };
      mockedGenerateObject.mockResolvedValue(mockResult);
  ...
  describe('Response Format Differences', () => {
    it('should extract object from generateObject result, not return full result', async () => {
      const mockResult = {
        object: { text: 'extracted completion' },
      ...
      // Key difference from www version: returns result.object, not result
      expect(data).toEqual({ text: 'extracted completion' });
      expect(data).not.toHaveProperty('usage');
      expect(data).not.toHaveProperty('finishReason');
      expect(data).not.toHaveProperty('warnings');
    });

    it('should preserve text field from object property', async () => {
      const mockResult = {
        object: { text: 'important text content' },
      };
      mockedGenerateObject.mockResolvedValue(mockResult);
      ...
      expect(data).toEqual({ text: 'important text content' });
      expect(data.text).toBe('important text content');
    });
  });
  ...
  describe('Concurrent Requests', () => {
    it('should handle multiple concurrent requests independently', async () => {
      ...
      expect(data[0]).toEqual({ text: 'completion 1' });
      expect(data[1]).toEqual({ text: 'completion 2' });
      expect(data[2]).toEqual({ text: 'completion 3' });
