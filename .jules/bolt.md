...
## 2024-05-15 - [O(1) Map Lookups in Render Loops]
**Learning:** For static lookup tables in React components (e.g., `languages` in `CodeBlockCombobox` or `items` in toolbar buttons), using O(N) `Array.find` calls on every render cycle causes unnecessary overhead, especially for long lists like syntax highlighting languages.
**Action:** Replace O(N) `Array.find` calls with O(1) `Map.get` lookups by defining a module-level `Map` derived from the source array. This is especially effective in `useMemo` hooks or render loops.
