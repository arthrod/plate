...
### January 20 #28.4
- `block-discussion`: Count discussion activity by actual comment volume (including replies) while keeping suggestions as separate track-change items.
- `export-toolbar-button`: Export only `comment_*` marks as DOCX comments; keep suggestion insertions as track changes.
- `import-toolbar-button`: Reset suggestion/comment path maps after DOCX import to avoid stale counting state.
