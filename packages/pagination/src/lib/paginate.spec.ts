...
it('returns one empty page for an empty doc', () => {
  const pages = paginate([], RECT, CTX, fixedHeight(0));
...
it('packs blocks until the content budget is exceeded', () => {
  ...
  const pages = paginate(doc, RECT, CTX, fixedHeight(40));
...
it('flushes on a manual page-break void', () => {
  ...
  const pages = paginate(doc, RECT, CTX, fixedHeight(20));
...
it('puts an oversized block on its own page', () => {
  ...
  const pages = paginate(doc, RECT, CTX, measure);
...
it('skips top-level header/footer/footnote-definition', () => {
  const doc = [
...
it('allocates footnote definitions to the page that references them', () => {
  ...
  const pages = paginate([refBlock, block('b')], RECT, CTX, fixedHeight(40));
