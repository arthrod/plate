...
import "./mammoth.js/mammoth.browser.js";
...
const getMammothModule = (): MammothModule => {
	const maybeMammoth = (globalThis as { mammoth?: unknown }).mammoth;

	if (
		!maybeMammoth ||
		typeof maybeMammoth !== "object" ||
		typeof (maybeMammoth as { convertToHtml?: unknown }).convertToHtml !==
			"function"
	) {
		throw new Error(
			"mammoth browser bundle did not initialize correctly (globalThis.mammoth missing).",
		);
	}

	return maybeMammoth as MammothModule;
};

/** Export mammoth for direct access if needed */
export const mammoth = getMammothModule();
