...
function DocumentConversion(options, comments) {
	...
	// Per-changeId occurrence counters to make multi-paragraph tokens unique
	var changeIdOccurrenceCounters = {};

	function resetTrackedChangeCounters() {
		trackedChangeIdCounter = 1;
		changeIdOccurrenceCounters = {};
	}

	function nextTrackedChangeOccurrence(changeId) {
		var occurrence = changeIdOccurrenceCounters[changeId] || 0;
		changeIdOccurrenceCounters[changeId] = occurrence + 1;
		return occurrence;
	}

	resetTrackedChangeCounters();
	...
	function convertToHtml(document) {
		// Reset counters for each document conversion to prevent state leakage.
		resetTrackedChangeCounters();
		var messages = [];
	...
	var elementConverters = {
		...
		inserted(element, messages, options) {
			...
			var occurrence = nextTrackedChangeOccurrence(changeId);
