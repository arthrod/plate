...
var documentMatchers = require("./document-matchers.ts");
...
var createBodyReaderForTests = require("./testing.ts").createBodyReaderForTests;
...
function isImage(options) {
    ...
    delete matcherOptions.buffer;
    var matcher = hasProperties(Object.assign({type: "image"}, matcherOptions));
    if (options.buffer) {
        return allOf(
            matcher,
