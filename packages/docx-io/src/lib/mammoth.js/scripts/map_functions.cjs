...
function searchFunctionInCodebase(functionName) {
  try {
    ...
    const patterns = [
      ...
      `\\b${functionName}\\s*:`,                   // object method
    ];

    for (const pattern of patterns) {
      try {
        const cmd = `grep -r "${pattern}" "${LIB_DIR}" --include="*.ts" --include="*.js" 2>/dev/null | head -3`;
...
function generateMapping() {
  ...
  console.log(`OLD_IMPL_DIR: ${OLD_IMPL_DIR}`);
  console.log(`LIB_DIR: ${LIB_DIR}`);

  const sampleSize = Math.min(oldFiles.length, 50);
  for (let i = 0; i < sampleSize; i++) { // Sample first 50
    const file = oldFiles[i];
    const funcName = extractFunctionName(file);

    if (i % 10 === 0) {
      console.log(`  Progress: ${i}/${sampleSize}`);
    }
  ...
  mapping.summary = {
    found,
    notFound,
    percentage: ((found / sampleSize) * 100).toFixed(2) + '%',
    note: `Sample of first ${sampleSize} files out of ${oldFiles.length}`
  };
